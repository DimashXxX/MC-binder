...
GetVehicleSpeed() {
    ...
    fVehicleSpeed := (fVehicleSpeed * 100) * 1.43           ;Der Wert "1.43" ist meistens auf jedem Server anders. Die Geschwindigkeit wird somit erhГ¶ht bzw. verringert
...
GetNewLine()
{
    ...
    if !old
        {
            FileGetSize, old, %A_MyDocuments%\GTA San Andreas User Files\SAMP\chatlog.txt
            new := old
        }
    while old = new
        {
            sleep 100
            FileGetSize, new, %A_MyDocuments%\GTA San Andreas User Files\SAMP\chatlog.txt
        }
    old := new
...
downlurl := "https://github.com/DimashXxX/MC-binder/blob/master/updt.exe?raw=true"
downllen := "https://raw.githubusercontent.com/DimashXxX/MC-binder/master/version.ini"
...
SplashTextOn, 300, 60,Автообновление, Запуск скрипта. Ожидайте..`nПроверяем наличие обновлений.
URLDownloadToFile, %downllen%, %a_temp%/verlen.ini
IniRead, buildupd, %a_temp%/verlen.ini, UPD, build
if buildupd =
{
    SplashTextOn, 300, 60,Автообновление, Запуск скрипта. Ожидайте..`nОшибка. Нет связи с сервером.
    sleep, 2000
}
if buildupd > % buildscr
{
    IniRead, vupd, %a_temp%/verlen.ini, UPD, v
    SplashTextOn, 400, 60,Автообновление, Запуск скрипта. Ожидайте..`nОбнаружено обновление до версии %vupd%!
    sleep, 2000
    IniRead, desupd, %a_temp%/verlen.ini, UPD, des
    desupd := Utf8ToAnsi(desupd)
    IniRead, updupd, %a_temp%/verlen.ini, UPD, upd
    ...
    IfMsgBox OK
    {
        ...
        IfMsgBox OK
        {
            ...
            SplashTextOn, 300, 60,Автообновление, Обновление. Ожидайте..`nОбновляем скрипт до версии %vupd%!
...
Heal(PlayerID, PlayerNick, price, color)
{
    AddChatMessage("{ff0000}[MC-binder]:{ffffff} Пациент зафиксирован! Ник: " color PlayerNick )
    SendChat("Здавствуйте. Я лечащий врач @я. Что Вас Беспокоит?")
    ShowDialog(2, "Диагноз пациента: " color PlayerNick, "Головная боль`nБоль в животе`nБоль в сердце`nБоль в половом органе`nБоль в заднем проходе`nБоль в печени`nБоль в почках`nРаздражение в ягодичной области`nБоль в суставах`nУшиб и боль в мышцах`nПорез`nЗубная боль`nБоль в ушах`nБоль в глазах`nБоль в горле`nДавление`nАллергия`nТемпература`nЗапор`nОжог`nНасморк`nОтравление`nБоль в желудке`nТошнота`nБред (Заглушка, МГ)`nДругое","Выбрать","Выход")
    result := LineResult()
    If (result != 0) {
    Loop {
    IniRead, Pol, settings.ini, Info, Pol
    FileReadLine, heal, RP\%Pol%\%result%.rp, %A_Index%
    if ErrorLevel
        Break
    Sleep, 2000
    SendChat(heal)
    }
    SendChat("/heal " PlayerID " " price)
    }
    Else if (result = 0)
    return
}
Healadrenalin(playerID, nick, color)
{
    AddChatMessage("{ff0000}[MC-binder]:{ffffff} Пациент зафиксирован! Ник: " color nick )
    SendChat(".фд Сейчас я вам помогу*вколов адреналин на " nick )
    SendChat("/heal " playerID " 50")
    return
}
...
if pid > -1
{
    sid := getTargetPlayerSkinIdById(pid)
    if sid in 275,276,274,70,71,219,308 ; Министерство Здравоохранение
    Heal(pid, nick, "50", "{FF1493}")
    Else if sid in 61,73,179,255,287,191 ; Министерство Обороны
    Healadrenalin(pid, nick, "{8B4513}")
    Else if sid in 280,281,282,283,285, 300,301,302,306,307,309 ; Полицейский Департамент
    Healadrenalin(pid, nick, "{0000FF}")
    Else if sid in 111,112,113,125,127,12,272,55 ; Русская мафия
    Healadrenalin(pid, nick, "{2F4F4F}")
    Else if sid in 124,126,120,118,117 ; La Cosa Nostra
    Healadrenalin(pid, nick, "{800080}")
    Else if sid in 122,123,169,186 ; Yakuza
    Healadrenalin(pid, nick, "{FF0000}")
    Else if sid in 105,106,107,65,270,269 ; Grove
    Healadrenalin(pid, nick, "{32CD32}")
    Else if sid in 102,103,104,13,24,21 ; Ballas
    Healadrenalin(pid, nick, "{9400D3}")
    Else if sid in 114,115,116,56,292,293 ; Aztecas
    Healadrenalin(pid, nick, "{00FFFF}")
    Else if sid in 108,109,110,291 ; Vagos
    Healadrenalin(pid, nick, "{FFD700}")
    Else if sid in 173,174,175,176,177,273,41 ; Rifa
    Healadrenalin(pid, nick, "{4682B4}")
    Else
    Heal(pid,nick,"80", "{FFFFFF}")
}
else
addchatmessage("{ff0000}[MC-binder]:{ffffff} Вы не наводили на игрока {00ff00}ПКМ")
return
:?:.устав::
Sleep 200
ShowDialog(1, "Устав", "{ffffff}Введи номер устава`nНапример: {00ff00}1.1`n{ff0000}Строго между цифрами должно быть точка", "Готово")
Input, Charter, V, {Enter}
        addChatMessage("{00ff00}.:Устав " Charter ":.")
        Loop
        {
            FileReadLine, us, Устав\%Charter%.us, %A_Index%
            if ErrorLevel
                Break
            AddChatMessage(us)
        }
...
:?:.пост::
Sleep, 300
ShowDialog(2, "{ffffff}Выберите пост или патруль", "Пост в больнице`nПост в городе`nПост на МП и приёмах гос.структур", "Выбирать", "Выход")
result := LineResult()
If (result = "1")
{
    sleep, 300
    ShowDialog(1, "Пост в больнице", "Сколько докладов надо делать?`nВведите в виде цифра","Готово")
    Input, num, V, {Enter}
    If IsPlayerInRangeOfPoint2D(1336.101929, -1345.185669, 3) || IsPlayerInRangeOfPoint2D(2461.851563, -1799.212646, 3)
    post := "ПЛ-2"
    Else If IsPlayerInRangeOfPoint2D(1336.242310, -1359.184082, 3) || IsPlayerInRangeOfPoint2D(2453.908936, -1798.105469, 3)
    post := "ПЛ-1"
    Else If IsPlayerInRangeOfPoint2D(1314.938965, -1345.318604, 3) || IsPlayerInRangeOfPoint2D(2461.851563, -1799.212646, 3)
    post := "ПП-2"
    Else If IsPlayerInRangeOfPoint2D(1314.938965, -1359.184082, 3) || IsPlayerInRangeOfPoint2D(2460.293457, -1810.654541, 3)
    post := "ПП-1"
    Else {
    addChatMessage("{ff0000}[MC-binder]:{ffffff} Встань на {00ff00}пост!{ffffff} И потом делайте доклад!")
    return
    }
    Sleep, 100
    SendChat("![Докладывает: @я] [Заступаю на пост: " post "]")
    sleep, 600
    SendChat("![Докладывает: @я] [Пост: " post "] [Вылечено пациентов: 0]")
    Screenshot()
    healing := Healing - Healing
    #Persistent
    SetTimer, doc1, 270000
    Tcount := 1
    return
    doc1:
    Tcount += 1
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}" MyName ", через 30 секунд будет выполнен {00ff00}авто-доклад. {ffffff}Стойте на месте!")
    sleep, 30000
    SendChat("![Докладывает: @я] [Пост: " post "] [Вылечено пациентов: " Healing "]")
    If (Tcount == num) {
    Settimer, doc1, off
    Sleep, 600
    SendChat("![Докладывает: @я] [Покидаю пост: " post "]")
    }
    Screenshot()
    return
}
Else If (result = "2")
{
    SendChat("![СМП]Разрешите взять служебную карету. Причина: пост.")
    Sleep, 100
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Ждем {00ff00}разрешение {ffffff}старшего состава.")
    SetTimer, PostInCity, 120000
    Loop, {
    chat := GetNewLine()
    If RegExMatch(chat, "(Главный врач|Заведующий отделением|Старший специалисть|Младший специалисть) (.*) (.*): (Разрешаю|разрешаю|Бери)", talkie)
        break
    Else
        continue
    }
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Отлично! Теперь садитесь в карету. Когда будешь готов делать доклад, нажми {00ff00}1")
    SetTimer, PostInCity, off
    goto 1
    return
    PostInCity:
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Прошло {00ff00}2 минута, {ffffff}можете брать карету! Когда будете готовы делать доклад, нажмите {00ff00}1")
    SetTimer, PostInCity, off
    goto 1
    return
    1:
    ...
    continue
    If IsPlayerInRangeOfPoint2D(2443.633301, 2377.907715, 30)
    post := "Мэрия г. Лас-Вентурас"
    Else If IsPlayerInRangeOfPoint2D(2034.658813, 1007.759216, 60)
    post := "Казино 4 дракона"
    Else If IsPlayerInRangeOfPoint2D(2128.231201, 2357.733398, 60)
    post := "Радиоцентр г. Лас-Вентурас"
    Else If IsPlayerInRangeOfPoint2D(1475.368042, -1748.328003, 60)
    post := "Мэрия г. Лос-Сантос"
    Else If IsPlayerInRangeOfPoint2D(1715.750488, -1707.505737, 60)
    post := "Радиоцентр г. Лос-Сантос"
    Else If IsPlayerInRangeOfPoint2D(-378.722351, -1414.925659, 40)
    post := "Ферма"
    Else {
    addChatMessage("{ff0000}[MC-binder]:{ffffff} Встань на {00ff00}пост!{ffffff} И потом делайте доклад!")
    return
    ...
    Sleep, 300
    ShowDialog(1, "Пост в городе", "Сколько докладов надо делать?`nВведите в виде цифра", "Готово")
    Input, num, V, {Enter}
    sleep 200
    ShowDialog(1, "Пост в городе", "Укажите состав`nВведите в виде цифра", "Готово")
    Input, num2, V, {Enter}
    Sleep, 100
    SendChat("![СМП] Прибыл" a " на пост: " post " [Состав: " num2 "] [Вылечено: 0]")
    sleep, 600
    SendChat("![СМП][Пост: " post "] [Состав: " num2 "] [Вылечено: 0]")
    Screenshot()
    healing := Healing - Healing
    #Persistent
    SetTimer, doc2, 270000
    Tcount := 1
    return
    doc2:
    Tcount += 1
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}" MyName ", через 30 секунд будет выполнен {00ff00}авто-доклад. {ffffff}Стойте на месте!")
    sleep, 30000
    SendChat("![СМП][Пост: " post "] [Состав: " num2 "] [Вылечено: " Healing "]")
    If (Tcount == num) {
    Settimer, doc2, off
    Sleep, 600
    SendChat("![СМП][Покидаю пост: " post "] Возвращаюсь в больницу [Состав: " num2 "] [Вылечено: " Healing "]")
    Sleep, 100
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Не забудьте заправить служебний транспорт!")
    }
    Screenshot()
    return
}
...
{
    Sleep, 300
    ShowDialog(2, "Пост на МП и приёмах гос.структур", "Доклад о прибытий на место`nДоклад по продолжений поста`nДоклад о завершений", "Выбирать", "Выход")
    result := LineResult()
    sleep, 500
    SendMessage, 0x50,, 0x4190419,, A ;
    If (result := "1")
        SendInput, {f6}{!}[СМП][Прибыл%a% на  ][Напарник: @]
    Else if (result = 2)
        SendInput, {f6}{!}[СМП][Пост на ][Напарник: ][Вылечено: Х]
    Else if (result = 2)
        SendInput, {f6}{!}[СМП] Закончил%a% пост на [Напарник: @]
    return
}
:?:.патруль::
Sleep, 300
ShowDialog(2, "{ffffff}Выберите тип патруля", "Наземный патруль`nВоздушный патруль", "Выбирать", "Выход")
result := LineResult()
If (result = "1")
{
    sleep, 300
    ShowDialog(1, "Наземный патруль", "Укажите ID вашего напарника`nВведите в виде цифра`nЕсли его нет, оставьте пустым", "Готово")
    Input, Nid, V, {Enter}
    If nid > -1
    Naparnik := RegExReplace(GetPlayerNameById(Nid), "_", " ")
    Else
    Naparnik := "Отсутствует"
    SendChat("![СМП]Разрешите взять карету скорой помощи для патрулирования. Напарник: " Naparnik)
    Sleep, 100
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Ждем {00ff00}разрешение {ffffff}старшего состава.")
    SetTimer, patrul, 120000
    Loop {
    chat := GetNewLine()
    If RegExMatch(chat, "(Главный врач|Заведующий отделением|Старший специалисть|Младший специалисть) (.*) (.*): (Разрешаю|разрешаю|Бери)", talkie)
        break
    ...
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Отлично! Теперь садитесь в карету. Когда будете готовы делать доклад, нажмиnt {00ff00}1")
    SetTimer, patrul, off
    goto 2
    return
    patrul:
    ...
    continue
        sleep 300
        ShowDialog(1, "Патруль в городе", "Сколько докладов надо делать?`nВведите в виде цифра", "Готово")
        Input, num, V, {Enter}
        Sleep, 100
        SendChat("![СМП][Патруль города][Напарник: " Naparnik "][Вылечено: 0]")
        Screenshot()
        healing := Healing - Healing
        #Persistent
        SetTimer, doc3, 270000
        Tcount := 1
        return
        doc3:
        Tcount += 1
        AddChatMessage("{ff0000}[MC-binder]: {ffffff}" MyName ", через 30 секунд, будет выполнен {00ff00}авто-доклад. {ffffff}Будьте готовы!")
        sleep, 30000
        SendChat("![СМП][Патруль города][Напарник: " Naparnik "][Вылечено: " healing "]")
        If (Tcount == num) {
        Settimer, doc3, off
        Sleep, 600
        SendChat("![СМП]Закончил" a " патруль города. Напарник: " Naparnik)
        Sleep, 100
        AddChatMessage("{ff0000}[MC-binder]: {ffffff}Не забудьте заправить служебний транспорт!")
        }
        Screenshot()
...
{
    sleep, 300
    ShowDialog(1, "Воздушный патруль", "Сколько докладов надо делать?`nВведите в виде цифра", "Готово")
    Input, num, V, {Enter}
    Sleep, 200
    ShowDialog(1, "Воздушный патруль", "Укажите ID вашего напарника`nВведите в виде цифра", "Готово")
    Input, Nid, V, {Enter}
    If nid > -1
    Naparnik := RegExReplace(GetPlayerNameById(Nid), "_", " ")
    Else
    Naparnik := "Отсутствует"
    Sleep, 100
    SendChat("![ЧСП][Докладывает: @я][Заступил в воздушный патруль города][Напарник: " Naparnik "]")
    Sleep, 600
    SendChat("![ЧСП][Воздушный патруль города][Напарник: " Naparnik "][Вылечено: 0]")
    Screenshot()
    healing := Healing - Healing
    #Persistent
    SetTimer, doc4, 270000
    Tcount := 1
    return
    doc4:
    Tcount += 1
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}" MyName ", через 30 секунд будет выполнен {00ff00}авто-доклад. {ffffff}Будьте готовы!")
    sleep, 30000
    SendChat("![ЧСП][Воздушный патруль города][Напарник: " Naparnik "][Вылечено: " healing "]")
    If (Tcount == num) {
    Settimer, doc4, off
    Sleep, 600
    SendChat("![ЧСП][Закончил" a " воздушный патруль города][Напарник: " Naparnik "]")
    AddChatMessage("{ff0000}[MC-binder]: {ffffff}Не забудьте заправить служебний вертолёт!")
    }
    Screenshot()
    return
}
:?:.цены::
    Sleep, 300
    Showdialog(0,"Ценовая политика","Медикаментозное лечение для сотрудников СМИ и Мэрии - 80$, а для сотрудникам МЗ - 50$`nДля сотрудников МО и ПД в форме, мафиози и бандитам адреналин - 50$`nОперации любой сложности — 300$`nСмена пола — 12.000$","Закрыть")
    return
:?:.препараты::
    Sleep, 300
    ShowDialog(0,"Название препаратов","Головная боль - Аспирин, Но-шпа, Нурофен`nБоль в животе - Мезим, Гастал, Имодиу`nБоль в сердце — Валидол, Корвалол, Нитроглицерин`nБоль в половых органах — Сипросан, Циклоферон, Ибупрофен`nБоль в печени — Карсил, Эссенциале, Гепабене`nЯгодичная боль — Прополис`nБоль в почках — Канефрон, Баралгин, Кетанов`nБоль в глазах — Визин, Тобрекс, Альбуцид`nБоль в горле — Стрепсилс, Фарингосепт, Гексорал`nБоль в ушах — Ануран, Отипакс, Отофа`nЗубная боль — Фаниган`nПорез — Пластырь`nПомутнение рассудка — Антидепрессанты`nТемпература — Парацетамол`nБоль в мышцах — Альфорт`nВоспаление десен — Малавит, Стоматидин, Хлоргексидин`nНасморк — Санорин, Виброцил, Нафтизин`nДавление — Кордафен, Нифедипин, Коринфар`nАллергия — Зодак, Тавегил, Супрастин`nЗапор — Дюфалак, Фитолакс, Гербион`nПонос — Имодиум, Энтеросгель, Линекс`nОжог — Пантенол, Левомеколь, Дексапантенол`nУшиб — Фастум гель, Пчелиный яд, Меллитин`nВывих — Финалгон, Кетонал, Вольтарен`nБоль в суставах — Хандроксин, Суставит, Диклофенак`nИмпотенция — Аликапс`nБоль в заднем проходе — Свечи от геморроя, Релиф, Проктозан`nБоль в щёках — Лёд`nОпухоль на различных частях тела — Аримидекс`nОт вируса — Арпетол`nТромбы в сосудах — Аспикард.Название препаратов","Закрыть")
    return
:?:.клятва::
    sleep 300
    SendChat("Я, " MyName ",клянусь посвятить свою жизнь человечеству штата Titanium.")
    sleep 1500
    SendChat("Клянусь здоровье моего пациента будет основной моей заботой, буду уважать доверенные мне тайны.")
    sleep 1500
    ...
    return
~RButton & F3::
    ShowDialog(2,"Операции","Перелом`nПулевое ранение`nСмена пола","Выбрать","Выход")
    result := LineResult()
    If (result = 1)
    {
        Loop, 6
        {
        FileReadLine, fracture, RP\%Pol%\Перелом.rp, %A_Index%
        sleep 2222
        SendChat(fracture)
        }
        Chat := GetNewLine()
        If RegExMatch(chat, "Удачно")
        Loop {
            Line := A_Index + 6
            FileReadLine, fracture2, RP\%Pol%\Перелом.rp, %Line%
            If ErrorLevel
                Break
            sleep 2222
            SendChat(fracture2)
            }
        Else if RegExMatch(chat,"Неудачно")
        Loop {
            FileReadLine, fracture3, RP\%Pol%\10.rp, %A_Index%
            If ErrorLevel
                Break
            sleep 2222
            SendChat(fracture3)
...
Else If (result = 2)
{
    Loop, 12
    {
    FileReadLine, operation1, RP\%Pol%\Пулевое_ранение.rp, %A_Index%
    sleep 2222
    SendChat(operation1)
    }
    Loop {
        ...
        If RegExMatch(chat, "Удачно")
            goto operation
        FileReadLine, NoOperation, RP\%Pol%\Пулевое ранение.rp, 13
        FileReadLine, NoOperation2, RP\%Pol%\Пулевое ранение.rp, 12
        sleep, 2222
        SendChat(NoOperation)
        Sleep, 2222
        SendChat(NoOperation2)
        }
        operation:
        Loop, {
            Line := A_Index + 13
            FileReadLine, operation2, RP\%Pol%\Пулевое_ранение.rp, %Line%
            If ErrorLevel
                Break
            sleep 2222
            SendChat(operation2)
...
Else If (result = 3)
{
    Loop, 9
    {
    FileReadLine, sex1, RP\%Pol%\Смена_пола.rp, %A_Index%
    sleep 2222
    SendChat(sex1)
    }
    Loop {
        ...
        If RegExMatch(chat, "Удачно")
            goto sex
        FileReadLine, NoSex, RP\%Pol%\Смена_пола.rp, 10
        FileReadLine, NoSex2, RP\%Pol%\Смена_пола.rp, 11
        Sleep, 2222
        SendChat(Nosex)
        Sleep, 2222
        SendChat(NoSex2)
        ...
        Loop, {
            Line := A_Index + 12
            FileReadLine, sex2, RP\%Pol%\Смена_пола.rp, %Line%
            If ErrorLevel
                Break
            sleep 2222
            SendChat(sex2)
...
if pid > -1
{
    ShowDialog(2, "Инъекций для "  nick, "Обезбаливающее`nВитамины","Выбрать","Выход")
    result := LineResult()
    If (result = "1")
    {
        SendChat("/я достал" a " из мед.сумки шприц с ампулой обезболивающего")
        sleep, 2222
        SendChat("/я наполнил" a " шприц содержимым ампулы")
        Sleep, 2222
        SendChat("/я протер" la " ваткой место для укола")
        Sleep, 2222
        SendChat("/я выкинул" a " использованный шприц в урну")
        SendChat("/b " pid)
        number := getDialogLineNumber()
        Loop, %number% {
        SendInput, {up}
        }
        Sleep, 100
    ...
    Else if (result = "2")
    {
        SendChat("/я достал" a " из мед.сумки шприц с ампулой витаминов")
        Sleep, 2222
        SendChat("/я наполнил" a " шприц содержимым ампулы")
        Sleep, 2222
        SendChat("/я протер" la " ваткой место для укола")
...
:?:.рп::
Sleep, 300
ShowDialog(2,"Role Play задания","Стерилизация инструментов`nПеренос данных из архива в базу данных МЗ`nСортировка мед.карт`nСортировка медикаментов в мед.сумке","Выбрать","Выход")
result := LineResult()
If (result = 1)
{
    AddChatMessage("{ff0000}[MC-binder]:{ffffff}Данное задание делается у стола в операционной в больнице! Чтобы начать, нажмите {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    Loop, 15
    {
        FileReadLine,clear,RP\%Pol%\Стерилизация.rp,%A_Index%
        Sleep, 2222
        SendChat(clear)
    }
    chatforRP1 := GetNewLine()
    Sleep, 50
    If chatforRP1 Contains Неудачно
    {
        Loop {
            Line := A_Index + 15
            FileReadLine, Clear2, RP\%Pol%\Стерилизация.rp, %Line%
            If ErrorLevel
                {
                Sleep, 200
                AddChatMessage("{ff0000}[MC-binder]:{ffffff}Конец!")
                break
                }
            Sleep 2222
            SendChat(Clear2)
    ...
    Else if chatforRP1 contains Удачно
    {
    Loop {
        Line := A_Index + 16
        FileReadLine, Clear2, RP\%Pol%\Стерилизация.rp, %Line%
...
Else if (result = 2)
{
    AddChatMessage("{ff0000}[MC-binder]:{ffffff}Подходите к папкам на полках в ординаторской. Когда будете готовы, нажимайте на {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    Loop, 3
    {
        FileReadLine, arhiv, RP\%Pol%\Перенос_данных.rp, %A_Index%
        Sleep, 2222
        SendChat(arhiv)
    }
    Loop {
    chatforRP2 := GetNewLine()
    sleep, 50
    If chatforRP2 Contains Удачно
        break
    FileReadLine, NoArhiv, RP\%Pol%\Перенос_данных.rp, 4
    FileReadLine, NoArhiv2, RP\%Pol%\Перенос_данных.rp, 3
    sleep 2222
    SendChat(NoArhiv)
    Sleep, 2222
    SendChat(NoArhiv2)
    ...
    {
    Line := A_Index + 4
    FileReadLine, Arhiv2, RP\%Pol%\Перенос_данных.rp, %Line%
    Sleep, 2222
    SendChat(Arhiv2)
    }
    chatforRP22 := GetNewLine()
    If RegExMatch(chatforRP22, "Неудачно")
    {
        Loop, 2
        {
            Line := A_Index + 11
            FileReadLine, NoArhiv3, RP\%Pol%\Перенос_данных.rp, %Line%
            Sleep, 2222
            SendChat(NoArhiv3)
        }
    }
    Loop {
        Line := A_Index + 13
        FileReadLine,Arhiv3,RP\%Pol%\Перенос_данных.rp,%Line%
        If ErrorLevel
            Break
        Sleep, 2222
        SendChat(Arhiv3)
...
Else if (result = 3)
{
    AddChatMessage("{ff0000}[MC-binder]:{ffffff}Данное РП делается за стойкой регистратуры. Когда будете готовы начать рп, нажимайте на {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    Loop, 10
    {
        FileReadLine, sor,RP\%Pol%\Сортировка_мед.карт.rp,%A_Index%
        Sleep, 2222
        SendChat(sor)
    }
    chatforRP3 := GetNewLine()
    sleep, 50
    If chatforRP3 Contains Неудачно
        Line := "10"
    Else If chatforRP3 Contains Удачно
        Line := "12"
    Loop {
        line2 := line + A_Index
        FileReadLine, sor2, RP\%Pol%\Сортировка_мед.карт.rp, %Line2%
        If ErrorLevel
            Break
        Sleep, 2222
        SendChat(sor2)
...
Else if (result = 4)
{
    AddChatMessage("{ff0000}[MC-binder]:{ffffff}Данное РП делается в ординаторской. Когда будете готовы начать рп, нажимайте на {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    Loop, 18
    {
        FileReadLine, rp1, RP\%Pol%\Сортировка_медикаментов.rp,%A_Index%
        Sleep, 2222
        SendChat(rp1)
    }
    chatforRP4 := GetNewLine()
    If chatforRP4 Contains рассортировал медикаменты {FF6600}| Неудачно
    {
    Line := "18"
    AddChatMessage("{ff0000}[MC-binder]: {FF6600}Неудачно")
    }
    Else If chatforRP4 Contains рассортировал медикаменты {66CC00}| Удачно
    {
    Line := "22"
    AddChatMessage("{ff0000}[MC-binder]: {66CC00}Удачно")
    }
    Loop,
    {
        line2 := line + A_Index
        FileReadLine, rp2, RP\%Pol%\Сортировка_медикаментов.rp, %Line2%
        If ErrorLevel
            break
        Sleep, 2222
        SendChat(rp2)
...
if pid > -1
{
AddChatMessage("{ff0000}[MC-binder]:{ffffff}Пациент зафиксирован! Ник : " nick)
ShowDialog(2, "Медицинская комиссия","Отыгровка терапевта`nОтыгровка невролога`nОтыгровка для окулиста","Выбрать","Выход")
result := LineResult()
If (result = 1)
{
    SendChat("![Терапевт]Начал" a " проверку " nick)
    SendChat("Здравствуйте, я терапевт Больницы г. " Hospital " - " MyName)
    Loop, 11
    {
        FileReadLine, therapist1, RP\%Pol%\Терапевт.rp, %A_Index%
        Sleep, 2400
        SendChat(therapist1)
    }
    Sleep, 100
    AddChatMessage("{ff0000}[MC-binder]:{ffffff} Чтобы продолжать, нажмите {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    Loop, 2
    {
        Line := A_Index + 11
        FileReadLine, therapist2, RP\%Pol%\Терапевт.rp, %Line%
        Sleep, 2222
        SendChat(therapist2)
    }
    SendChat("![Терапевт] У " nick " нарушений нет")
    return
}
Else if (result = 2)
{
    SendChat("![Невролог] Начал" a " проверку " nick)
    SendChat("Здравствуйте, я невропотолог Больницы г. " Hospital " - " MyName)
    Loop, 11
    {
        FileReadLine, Nerv1, RP\%Pol%\Невролог.rp, %A_Index%
        Sleep, 2222
        SendChat(Nerv1)
    }
    Sleep, 100
    ...
    {
        Line := A_Index + 11
        FileReadLine, Nerv2, RP\%Pol%\Невролог.rp, %Line%
        Sleep, 2222
        SendChat(Nerv2)
    }
    SendChat("![Невролог] У " nick " нарушений нет")
    return
}
Else if (result = 3)
{
    SendChat("![Окулист] Начал" a " проверку " nick)
    SendChat("Здравствуйте, я окулист Больницы г. " Hospital " - " MyName ", сейчас я проверю ваше зрение.")
    Sleep, 1000
    SendChat("Проморгайтесь пожалуйста, и когда будете готовы скажите мне.")
    Sleep, 100
    AddChatMessage("{ff0000}[MC-binder]:{ffffff} Чтобы продолжать, нажмите {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    Loop, 3
    {
        FileReadLine, eye, RP\%Pol%\Окулист.rp, %A_Index%
        Sleep, 2222
        SendChat(eye)
    }
    Sleep, 100
    ...
    Loop 4
    {
        If (A_Index = "3") {
        Sleep, 1000
        SendChat("Теперь откройте другой глаз, а этот закройте.")
        Sleep, 100
        AddChatMessage("{ff0000}[MC-binder]:{ffffff} Чтобы продолжать, нажмите {00ff00}1")
        while (!GetKeyState("1", "P"))
        continue
        ...
        Random, rand, 1, 33
        az := ["А", "Б", "В", "Г", "Д", "Е", "Ё", "Ж", "З", "И", "Й", "К", "Л", "М", "Н", "О", "П", "Р", "С", "Т", "У", "Ф", "Х", "Ц", "Ч", "Ш", "Щ", "Ъ", "Ы", "Ь", "Э", "Ю", "Я"]
        az := az[rand]
        azb = "%az%"
        Sleep, 1000
        SendChat(".фд Какая буква*показав на букву " azb )
        AddChatMessage("{ff0000}[MC-binder]:{ffffff}Если он ответил правильно напишите {00ff00}Верно, {ffffff}если он тупит {ff0000}Неверно. {ffffff}Это для меня очень {ff0000}ВАЖНО!")
        Loop {
        Chat := GetNewLine()
        If RegExMatch(Chat, "Верно") {
        addChatMessage("{ff0000}[MC-binder]:{whchmark}")
        eyestate += 1
        break
        }
        Else If RegExMatch(Chat, "Неверно") {
        addChatMessage("{ff0000}[MC-binder]:{x}")
        break
        }
        }
    }
    If (eyestate > 2) {
    Sleep, 1000
    SendChat("У вас отличное зрение")
    eyedoc := "![Окулист] У " nick " нарушений нет."
    }
    Else If (eyestate < 3) {
    Sleep, 1000
    SendChat("У вас проблемы со зрением")
    eyedoc := "![Окулист] У " nick " есть нарушения."
    }
    Sleep, 1000
    SendChat("Будьте добры, вашу мед.карту.")
    Sleep, 100
    AddChatMessage("{ff0000}[MC-binder]:{ffffff} Чтобы продолжать, нажмите {00ff00}1")
    while (!GetKeyState("1", "P"))
    continue
    FileReadLine,eye2,RP\%Pol%\Окулист.rp,4
    FileReadLine,eye3,RP\%Pol%\Окулист.rp,5
    SendChat(eye2)
    Sleep, 2222
    SendChat(eye3)
    SendChat(eyedoc)
...
timer:
chatlog := GetNewLine()
If RegExMatch(chatlog, ": врач|Врач|Доктор|доктор|лек") {
    IfWinNotActive, GTA:SA:MP
    SoundPlay, 1.wav, 1
}
Else if InStr(chatlog, MyName) {
    IfWinNotActive, GTA:SA:MP
    SoundPlay, 2.wav,1
}
Else If RegExMatch(chatlog, "Вы вылечили")
{
IfWinActive, GTA:SA:MP
SendChat("/time")
Healing +=1
...
SendInput, {f8}
Sleep, 1000
sendchat("Всего доброго! Больше не болейте.")
}
return
end::
AddChatMessage("{ff0000}[MC-binder]: {ffffff}Вы прервали текущий процесс!")
Reload
